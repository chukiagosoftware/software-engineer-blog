...
description: "A deep dive into deploying a modern application on AWS EKS using Pulumi in Go, including VPC setup, EKS cluster configuration, VPC endpoints, and troubleshooting common issues."
draft: false
---
...
**Solution**: We added an S3 Gateway endpoint (`com.amazonaws.us-east-1.s3`) with a policy for `arn{{< emoji aws >}}s3:::prod-us-east-1-starport-layer-bucket/*`. A Pulumi bug—`ctx.Stack()` returning `prod` instead of `us-east-1`—misnamed the bucket as `prod-prod-`. Fixing it to `us-east-1` aligned the policy, resolving the 403.
